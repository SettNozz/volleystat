...
class FullPipeline:
    def __init__(self, model_path: str, support_img_path: str, yolo_model_path: str = None):
        """
        Initialize the full pipeline with YOLO person/face detection and UNet ball segmentation.
        ...
        print(f"Initialized full pipeline on {self.device}")
        print(f"Detection method: YOLO (persons + estimated faces)")
        print(f"Ball detection: Siamese UNet segmentation")
    ...
    def process_video_segments(self, input_video_path: str, output_dir: str, 
                              segment_duration: int = 8, num_segments: int = 3,
                              blur_strength: int = 15):
        """
        Process video and save short segments with full pipeline.
        ...
        for i, start_frame in enumerate(segment_starts):
            ...
            output_path = os.path.join(output_dir, f"pipeline_segment_{i+1:02d}_start_{start_frame:06d}.mp4")
...
def main():
    ...
    pipeline.process_video_segments(
